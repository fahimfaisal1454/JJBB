...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 14, textAlign: "center", marginBottom: 12, fontWeight: "bold", color: "#ffffff", backgroundColor: "#3a6791", padding: 8 },
  ...
  tableHeaderRow: { flexDirection: "row", backgroundColor: "#c8dafc", borderBottomWidth: 1, borderBottomColor: "#9cb4d8", borderBottomStyle: "solid" },
  ...
  amountHeader: { flex: 1, paddingVertical: 6, borderLeftWidth: 1, borderLeftColor: "#9cb4d8", borderLeftStyle: "solid", fontWeight: "bold", color: "#4a6fa5", textAlign: "center" },
  percentHeader: { flex: 1, paddingVertical: 6, fontWeight: "bold", color: "#4a6fa5", textAlign: "right", paddingRight: 8 },
  sectionHeaderRow: { flexDirection: "row", backgroundColor: "#a5bedb", borderBottomWidth: 1, borderBottomColor: "#6c88af", borderBottomStyle: "solid", marginTop: 10, marginBottom: 4 },
  sectionHeaderText: { flex: 1, padding: 6, fontWeight: "bold", color: "#252525" },
  sectionHeaderSpacer: { flex: 3 },
  row: { flexDirection: "row", borderBottomWidth: 1, borderBottomColor: "#ccc", borderBottomStyle: "solid" },
  ...
  cellAmount: { flex: 1, paddingVertical: 6, borderLeftWidth: 1, borderLeftColor: "#ddd", borderLeftStyle: "solid", textAlign: "right", color: "#252525" },
...
const ProfitLossPDF = ({ report }) => {
  ...
  // Fetch banner whenever selectedCategory changes
  useEffect(() => {
  ...
  const companyName = banner?.banner_title || selectedCategory?.name || "Business Name";
  ...
  const renderRow = (
    ...
    <View style={[styles.row, rowStyle]}>
      ...
      <Text style={[styles.cellAmount, bold && styles.boldText, !bold && styles.positivePercent, isAmountNegative && styles.greenText]}>
        {prev.toLocaleString()}
      </Text>
      <Text style={[styles.cellAmount, bold && styles.boldText, !bold && styles.positivePercent, isAmountNegative && styles.greenText]}>
        {curr.toLocaleString()}
      </Text>
      <Text style={[styles.cellPercent, bold && styles.boldText, isPercentNegative ? styles.negativePercent : styles.positivePercent]}>
        {percent}%
      </Text>
    </View>
  );
  ...
  const renderIncomeRows = () =>
    report.income.map((row, idx) =>
      renderRow(row.item, row.previous_year, row.current_year, row.percent_change, false, idx % 2 === 0 ? styles.rowIncomeLight : {})
    );

  const renderExpenseRows = () =>
    report.expenses.map((row, idx) =>
      renderRow(row.item, row.previous_year, row.current_year, row.percent_change, false, idx % 2 === 0 ? styles.rowExpensesLight : {})
    );
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.companyYearRow}>
          <Text style={styles.companyName}>
            {bannerLoading ? "(Loading banner...)" : companyName}
          </Text>
