...
export default function ExpensePage() {
  ...
  const loadCategories = async () => {
    try {
      const res = await AxiosInstance.get("cost-categories/"); // already used elsewhere
      ...
      <form onSubmit={onSubmit} className="flex items-end gap-4 flex-wrap">
        ...
        <div className="flex gap-3 mb-1">
          <button
            type="submit"
            disabled={saving}
            className="bg-blue-950 hover:bg-blue-700 text-white px-3 py-[6px] rounded-md cursor-pointer disabled:opacity-60"
          >
            {saving ? "Saving..." : "Save"}
          </button>
          <button
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full border border-collapse text-sm">
          ...
          <tbody>
            ...
            {expenses.length === 0 && (
              <tr>
                <td className="py-4 px-2 text-center text-slate-400" colSpan={6}>
                  No expenses yet
                </td>
              </tr>
            )}
